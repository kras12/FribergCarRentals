...
using FribergCarRentals.Shared.ViewModels.CarCategory;
...
namespace FribergCarRentals.Shared.ViewModels.Car
{
    ...
    /// </summary>
    public abstract class CreateCarViewModelBase : CarViewModelBase
    {
        #region Constructors
        ...
        {
            RentalStatuses = Enum.GetValues<RentalCarStatus>().Select(x => new CarRentalStatusViewModel((int)x, GetAttribute<EnumDatabaseValueAttribute>().Value, GetAttribute<EnumDatabaseValueAttribute>().Description).ToList());
            VehiclePropulsions = Enum.GetValues<VehiclePropulsionType>().Select(x => new VehiclePropulsionViewModel((int)x, GetAttribute<EnumDatabaseValueAttribute>().Value, GetAttribute<EnumDatabaseValueAttribute>().Description).ToList());
        }
